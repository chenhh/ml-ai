...
在許多書中使用字母$$A,B$$表示事件，使用$$\mathrm{P}(A|B)$$表示條件機率，這相對太抽象。另一套一套字母體系：$$H$$和$$E(D)$$，其中$$H$$假設或模型(hypothesis或model)，$$E$$為證據或資料)evidence或D=data）。這樣貝氏的推理過程可以表述為：通過不斷的收集證據$$E$$來強化對假設事件$$H$$的信心。

### 頻率派與貝氏派的思考方法

<mark style="color:blue;">貝氏機率中，機率是個人的一個主觀概念， 表明我們對某個事物是否發生的相信程度</mark>。

頻率學派定義機率是一件事發生的頻率， 這個頻率就代表某件事發生的可能性， 或者稱為客觀機率。
