...
<figure><img src="../.gitbook/assets/image (65).png" alt="" width="563"><figcaption><p>傳統統計學和貝氏統計的比較</p></figcaption></figure>

$$H/\theta$$和$$E/D$$，其中$$H/\theta$$為假設、模型或參數(hypothesis或model)，$$E/D$$為證據或資料(evidence或D=data）。

這樣貝氏的推理過程可以表述為：通過不斷的收集證據$$E$$/資料$$D$$來強化對假設事件/參數$$H/\theta$$的信心。

* 先驗分佈$$\mathrm{P}(\theta)$$或$$\mathrm{P}(H)$$：主觀機率，通常由經驗得出。因為尚未觀測到資料，所以稱為先驗。
*

### 頻率派與貝氏派的思考方法

<mark style="color:blue;">貝氏機率中，機率是個人的一個主觀概念， 表明我們對某個事物是否發生的相信程度</mark>。貝氏學派認為參數本身是一個隨機變數，可以有自己的分佈，先驗資訊可以幫助得到更好參數的估計和分佈情況。

頻率學派定義機率是一件事發生的頻率， 這個頻率就代表某件事發生的可能性， 或者稱為客觀機率。頻率學派認為參數是固定的，有一個確切的真值。

貝氏統計的優勢在於在資料較少的情況下，也可以進行推論，且資料越多，推論結果越準確。以及對所獲得的資訊可做出即時反應，自動升級「推論」的學習功能。

貝氏公式中，包含了先驗機率(主觀機率)，因此在頻率學派(客觀機率)中不易被接受。
...
* 頻率派推論與貝氏推論的中心都圍繞似然( likelihood)的概念。然而二者對似然提供的資訊之理解和解釋完全不同。

相異點：<mark style="background-color:red;">頻率派把未知參數</mark>$$\theta$$<mark style="background-color:red;">看成一個固定量，而貝氏派將</mark>$$\theta$$<mark style="background-color:red;">視為隨機變數，且隨機變數的分佈隨著觀察到的資料而調整</mark>。
...
<mark style="color:blue;">頻率派把需要推論的參數(向量)</mark>$$\theta$$<mark style="color:blue;">看成固定的未知參數。即</mark>$$\theta$$<mark style="color:blue;">是未知的固定值。同時樣本</mark>$$X$$<mark style="color:blue;">是隨機變數，所以頻率學派研究樣本空間，大部份的機率都是討論樣本</mark>$$X$$<mark style="color:blue;">的分佈。</mark>

<mark style="color:blue;">貝氏學派認為參數</mark>$$\theta$$<mark style="color:blue;">是隨機變數，而樣本</mark>$$X$$<mark style="color:blue;">是已觀察到的固定實現值，因此研究重點是</mark>$$\theta$$<mark style="color:blue;">的分佈。</mark>
...
<mark style="color:blue;">統計推論的主要任務是使用樣本估計總體</mark>。樣本有兩重性：
...
<figure><img src="../.gitbook/assets/image (64).png" alt="" width="375"><figcaption><p>貝氏推理</p></figcaption></figure>
...
值得注意的是，貝氏規則並沒有告訴我們我們的信念(beliefs)應該是什麼，它告訴我們在看到新訊息後應該如何改變信念。如果$$\mathrm{P}(\theta)$$ 和$$\mathrm{P}(X|\theta)$$ 代表一個理性人的信念，那麼貝氏規則就是更新這個人的信念的最佳方法。

在許多複雜的統計問題中，沒有明顯的非貝氏估計或推理方法。在這些情況下，貝氏規則可用於產生估計程序，並且可以使用非貝氏標準來評估這些程序的性能。。
...
在頻率派的推理中，顯著水平是其風險的指標。 其含義是：例如將顯著水平設定 $$\alpha$$，那麼如果用同一種方法，反覆使用假設檢驗的話，有$$\alpha$$的概率會得出錯誤的結論。

根據貝氏推理得出的結論的風險，其實就是「後驗機率」本身。

### 把貝氏機率工具用於建模

貝氏率是非常基礎的統計知識， 在心理學，經濟學， 神經科學等領域具有巨大潛力。因為這類問題的研究對像往往具有極高的不確定性， 通常是複雜系統。這就造成直接用物理學求分子結構解薛定諤方程的思路是不行的， 無法把人的行為像氫原子光譜一樣求解出來。

傳統統計的方式是做量表， 去統計所有可能的人的屬性和和它們的行為之間的聯絡， 然後求皮爾森係數。 但是這樣的方法雖然可以用， 但在量化和機器學習發展急速的今天還是必須做許多工作。

折衷的方法是貝氏建模。 貝氏建模非常善於處理“黑箱” 問題，對付這種不好精確預測但有些用到一點建模的東西很實用。貝氏建模可以很快的把實驗資料和理論做一個結合。只要有假設先驗， 有觀測資料， 有似然性函數， 就可以建立貝氏模型。這裡的似然性，經常是由我們的理論模型提供的，而貝氏的框架可以把這個模型的參數迅速的推導得出。這同時也意味著，如果手裡有幾個不同的侯選模型假設，給定觀測資料後，貝氏模型會迅速告訴你哪個模型比較合理。
...
[https://github.com/lixianmin/cloud/blob/master/writer/R/bayes.md](https://github.com/lixianmin/cloud/blob/master/writer/R/bayes.md)
