...
貝氏 在貝氏統計中，我們常常會遇到這樣的問題：給定一個先驗分佈（代表我們對引數的初始信念）和一些觀測資料，如何更新我們的信念，得到一個後驗分佈？

<mark style="color:red;">**共軛分佈是指先驗分佈和後驗分佈屬於同一分佈族的特殊情況**</mark>。可以直接通過解析公式得到後驗分佈，而不需要進行複雜的數值計算。
...
* **Beta分佈與二項分佈：** 當我們對<mark style="color:red;">**一個事件的發生機率**</mark>感興趣時，通常會使用Beta分佈作為先驗，而二項分佈作為似然函式。後驗分佈仍然是Beta分佈。
* **Gamma分佈與泊松分佈：** 當我們<mark style="color:red;">**對一個事件在單位時間內發生的次數**</mark>感興趣時，通常會使用Gamma分佈作為先驗，而泊松分佈作為似然函式。後驗分佈仍然是Gamma分佈。
* **正態分佈與正態分佈：** 當我們對一個<mark style="color:red;">**連續型隨機變數**</mark>感興趣時，通常會使用正態分佈作為先驗和似然函式。後驗分佈仍然是正態分佈。
...
雖然共軛分佈在貝氏統計中非常有用，但它也有一些限制：

* **選擇有限：** 並不是所有的先驗分佈都有共軛先驗。
* **可能過於簡單：** 共軛先驗可能過於簡單，無法完全捕捉真實的先驗知識。
