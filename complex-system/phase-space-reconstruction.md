...
1981年 Takens 提出**嵌入定理（delay embedding theorem, **[**wikipedia**](https://en.wikipedia.org/wiki/Takens's\_theorem)**）**：對於無限長，無噪聲的$$d^{'}$$維混沌吸引子的一維標量時間序列$$\{x(i), i=1,2,\ldots, n\}$$都可以在拓撲不變的意義下找到一個 $$d$$ 維的嵌入相空間，只要維數$$d \leq 2d^{'}+1$$。 **根據 Takens 嵌入定理，我們可以從一維混沌時間序列中重構一個與原動力系統在拓撲意義下一樣的相空間**，混沌時間序列的判定，分析和預測都是在這個重構的相空間中進行的，因此相空間的重構就是混沌時間序列研究的關鍵。
...
對於實際的混沌時間序列，從嵌入維數的最小值 $$2$$開始，計算虛假最臨近點的比例，然後逐漸增加維數$$d$$， 直到虛假最臨近點的比例小於 5% 或者虛假最臨近點的不再隨著$$d$$的增加而減少時，可以認為混沌吸引子已經完全打開，此時的$$d$$就是嵌入維數。在**相空間重構方面，虛假最臨近點法（FNN）被認為是計算嵌入維數很有效的方法之一**。

### &#x20;虛假最臨近點法的改進-Cao方法

虛假最臨近點法對數據的噪聲比較敏感，而且實際操作中需要選擇閥值 $$R_\tau$$具有很強的主觀性。此時 Cao Liangyue 提出了改進的 FNN 方法，此方法計算時只需要延遲時間$$\tau$$ 一個參數，用較小的數據量就可以求的嵌入維數$$d$$。

假設我們有時間序列$$x(1),\ldots, x(n)$$，那麼基於延遲時間$$\tau$$所構造的向量空間為$$y_i(d)=[x(i), \ldots, x(i+(d-1)\tau)] ~  1 \leq i \leq n-(d-1)\tau$$。

與虛假最臨近點類似，定義變量&#x20;

* $$a(i,d) = \| y_i(d+1) - y_{n(i,d)}(d+1) \|_{\infty} / \| y_i(d) - y_{n(i,d)}(d)\|_{\infty}, ~ 1 \leq i \leq n-d\tau$$
* $$y_i(d+1) = [x(i), \ldots, x(i+d\tau)]$$
* $$n(i,d) \in \{ 1,2,\ldots, n-d\tau \}$$是使得$$y_{n(i,d)}(d)$$在$$d$$維相空間中，在最大範數下與$$y_i(d)$$距離最近的向量，且$$n(i,d) \neq i$$，顯示$$n(i,d)$$由$$i$$與$$d$$決定。

定義$$\displaystyle E(d)=\frac{1}{n-d\tau} \sum_{i=1}^{n-d\tau} a(i,d)$$，則$$E(d)$$與延遲時間$$\tau$$與嵌入維度$$d$$有關。

為了發現$$d$$到$$d+1$$該函數的變化量，可計算$$E_1(d)=\frac{E(d+1)}{E(d)}$$，當$$d$$大於某個值$$d_0$$時，$$E_1(d)$$不再有大變化，那麼$$d_0+1$$則是我們所尋找的最小嵌入維數。

也可定義$$E_2(d)$$如下：

* $$\displaystyle E^{*}(d)=\frac{1}{n-d\tau}\sum_{i=1}^{n-d\tau} [x(i+d\tau)-x(n(i,d)+d\tau)]$$
* $$E_2(d)=\frac{E^{*}(d+1)}{E^{*}(d)}$$

對於隨機序列，數據之間沒有相關性，$$E_2(d)=1$$；對於確定性的序列，資料點之間的關系依賴於嵌入維數$$d$$的變化，總存在一些值使得$$E_2(d)\neq 1$$。

## 相空間的預測

通過前面的相空間重構過程，一個混沌時間序列$$x(1),\ldots, x(n)$$可以確定其延遲時間$$\tau$$和嵌入維數$$d$$形成$$n-(d-1)\tau$$個$$d$$維向量：

* $$\vec{y_1} = y_1(d)=[x(1), \ldots ,x(1+(d-1)\tau)]$$
* $$\vec{y_2} = y_2(d)=[x(2),\ldots, x(2+(d-1)\tau)]$$
* $$\vdots$$
* $$\vec{y}_{n-(d-1)\tau}=y_{n-(d-1)\tau}(d)=[x(n-(d-1)\tau), \ldots, x(n)]$$

這樣我們就可以在$$d$$維歐式空間$$\mathbb{R}^d$$建立一個動力系統的模型如下： $$\vec{y}_{i+1} =F(\vec{y}_{i})$$，其中$$F$$是一個連續函數。

### 區域最鄰近點預測法(one-nearest-neighborhood)

假設$$N=N-(D-1)\tau$$，根據連續函數的性質可以知道：如果$$\vec{y}_N$$與$$\vec{y}_j$$非常接近，則 非常接近，那麼$$\vec{y}_{N+1}$$與$$\vec{y}_{j+1}$$也是非常接近。因此可以 以用 $$x(j+1+(d-1)\tau)$$ 作為 $$x(N+1+(d-1)\tau)=x(n+1)$$ 的近似值。
