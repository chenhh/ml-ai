...
2. **簡單預測示例**：
   ...
   * 預測者在每個時間點 $$t$$ 給出預測$$\hat{p}_t \in \{0,1\}$$後，才知道$$y_t$$的值。
   * 預測者根據 $$N$$ 個專家$$\{f_{1,t}, f_{2,t}, \dots, f_{N,t}\}, ~f_{i,t} \in \{0,1\}$$的建議進行預測，<mark style="color:red;">目標是限制預測者的錯誤次數，即限制</mark>$$\sum_{t} \hat{p}_t \neq y_t$$的次數<mark style="color:red;">。</mark>

### 減半策略

假設存在至少一個專家從不犯錯，即$$\exists i \in N \ni f_{i,t} = y_{t} ~\forall t$$，但是我們只知道有這種很厲害的專家存在於專家群中，但是不知道是那一個專家。可以設計一個簡單預測策略，使其錯誤次數不超過 $$⌊log⁡_2N⌋$$。

* 初始權重分配：每個專家開始時被賦予相同的權重，$$w_j=1, \forall j=1,2,\dots, N$$。
* 簡單多數決：在每一期預測的時候，只考慮$$w_j=1$$的專家們，而$$\hat{p}_t$$之值為$$f_{j,t}$$中給出0或1較多者。
* 更新權重：之後得到答案$$y_t$$，將給出錯誤預測的專家們的權重設為0，即$$w_j=0, \text{ if } f_{j,t} \neq y_t$$。
* 進行下一輪預測，最後只會剩下全部預測正確的專家們。

令$$W_m$$為在犯了$$m$$次預測錯誤之後，專家們的權重總合。初始的時候$$m=0, W_0=N$$。

由於採取簡單多數決的預測，因此在預測者犯第$$m$$次錯誤時，在還沒預測錯誤的專家群中至少有一半還沒預測錯誤，即$$W_m \leq W_{m-1} /2, ~\forall m \geq 1, W_0=N$$。

整理後可得$$1 \leq N/2^m$$，即$$m \leq \lfloor \log_2 N \rfloor$$。

一般化的策略

* **預測策略**：預測器根據專家的建議進行預測，並在每次預測後更新專家的權重。
  * 如果某個專家的預測錯誤，其權重將被減少。
  * 如果某個專家從未犯錯，則其權重保持不變。
* **權重更新規則**：如果專家 $$k$$ 的預測錯誤，其權重 $$w_k$$ 將被乘以一個因子 $$β$$，其中 $$0<β<1$$ 是自由參數。
* **權重變化**：隨著預測次數的增加，權重會反映專家的預測準確性。
* **錯誤次數界限**：通過分析，可以得出預測器的錯誤次數與專家的最佳表現之間的關係。
  * 如果專家 $$i$$ 從未犯錯，則其權重始終保持最大。
  * 預測器的錯誤次數可以通過專家的錯誤次數來界線。
