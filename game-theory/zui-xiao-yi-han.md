...
因為玩家的行動集合$$\mathcal{A}_1 = \{a_1^1, a_1^2, \dots, a_1^P \}$$有$$P$$​個元素，可得$$P^2$$​個修改策略$$s_{t}^{i \rightarrow j}$$與成對遺憾(pairwise regret)，其中$$R_{s_{1, 1:T}}^{i \rightarrow i} =0, ~ i=1,2,\dots, P$$。

因此在時間$$t$$​時，這$$P^2$$​個修改策略可得$$P^2$$​個報酬，記為報酬向量$$\mathbf{u}_t \in \mathbb{R}^{P^2}$$。
