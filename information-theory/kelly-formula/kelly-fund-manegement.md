...
假設賭局勝率為$$p$$，賠率為$$b$$，一單位為1元，則下注1元的期望淨利為：$$p \times b+(1-p) \times (-1)$$。

整理後為$$p \times (1+b)-1$$<mark style="color:red;">，此為Kelly公式的分子部份</mark>。

* 若$$p \times (1+b) -1 > 0$$，(正期望值)則此賭局有利可圖(profitable)。
* 如$$p \times (1+b) -1 < 0$$，(負期望值)則此賭局無利可圖(non-profitable)。
* 注意上述的勝率$$p$$都是在無限次賭局$$T \rightarrow \infty$$才會逼近，有限次賭局時不一定成立。

### 期望值越高不一定下注比例越高

賭局Ａ：勝率$$p=50\%$$，賠率$$b=2$$，可得期望淨值為$$0.5 \times (1+2)-1=0.5$$。

賭局Ｂ：勝率$$p=1\%$$，賠率$$b=1000$$，可得期望淨值為$$0.01 \times (1+1000)-1=9.01$$。

<mark style="color:red;">賭局B的期望值雖然比較高，但是大部份都是輸的，平均玩100次才會贏1次，難以分配資金</mark>。

### 以期望值下注不合理

考慮單次賭局，勝率$$p$$，賠率$$b$$，下注資金比例$$0 \leq f \leq 1$$，目標是調整$$f$$以最大化報酬。

假設原始資金1元，，則資金期望值為$$g(f)=p \times (1+bf) + (1-p)  \times (1-f)= 1+f(p(1+b)-1)$$。

但$$g(f)$$為$$f$$為變數的一次線性函數，因此最大化$$f^{*} = \argmax_{f} g(f)=1$$，即全押資金。

分析上式：

* 在$$p=0.5$$，$$b=1.01$$時，$$g(f)=1+0.005f$$，即投入1元，獲勝可得0.005元，但輸了就賠1元，風險極高。
* 在$$p=0.01$$，$$b=1000$$時，$$g(f)=1+9.01f$$，即投入1元，獲勝可得9.01元，但輸了就賠1元。雖然期望淨利很高，但是勝率很低，只要輸一次就破產。
