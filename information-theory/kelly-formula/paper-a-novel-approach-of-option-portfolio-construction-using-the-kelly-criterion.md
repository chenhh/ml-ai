...
<figure><img src="../../.gitbook/assets/image (1) (1).png" alt="" width="421"><figcaption><p>當日的買/賣權不同履約價的價格</p></figcaption></figure>

long 9300call@90與short 9350@54，可形成多頭價差如下：

<figure><img src="../../.gitbook/assets/image (1) (1) (1).png" alt="" width="381"><figcaption><p>買權多頭價差(買call 9300@90, 賣call 9350@54，成本36元)最大獲利/虧損為14/36元。</p></figcaption></figure>
...
1. 履約價：買權或賣權有以下履約價$$\{p_1, p_2, \dots, p_n\}$$，其中$$p_i < p_j \forall i < j$$。
2. 持有期間：持有選擇權組合的期間，定義為$$\rho=Fri1343~Wed1330$$ (不考慮夜盤)。

Step1：計算多頭(空頭)價差投資組合在所有可能結果的損益。例如(多頭價差)計算買進$$p_i$$賣出$$p_j$$, $$i,j \in \{1,2,\dots, n\}, ~ i <j$$的可能損益。

Step2：從歷史資料中，計算TAIEX在持有期間$$p$$的報酬分佈。(註：可換成其它方法得到報酬分佈)。

Step3：由Step2得到的報酬分佈，由目前(Fri 13:30)的TAIEX指數推算下星期三13:30的指數價格分佈。

Step4：由選擇權組合配合預測的指數價格分佈計算損益得損益分佈。

Step5：根據 Vince 的最佳比例計算終端財富回報 (TWR)。然後，從 1%到 100%，依次計算各選擇權組合的報酬率。然後，選擇預期幾何報酬率最大的獲利組合。

輸出：最高預期報酬的選擇權組合，即買進買權$$p_i$$與賣出賣權$$p_j$$。

上述演算法可應用於各種期權組合，並計算其幾何回報。可以計算出所有組合的回報，並選擇出最有利可圖的組合。接下來我們展示尋找最有利可圖的選擇權差價的實驗。
...
<figure><img src="../../.gitbook/assets/image (1).png" alt="" width="525"><figcaption><p>多頭價差組合</p></figcaption></figure>
