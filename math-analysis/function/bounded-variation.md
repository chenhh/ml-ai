...
在處理積分$$\int f dg$$時，積分函數$$g$$的值域會切無限多段後，再進行加總。為了討論此切值域再加總的性質，所以討論有界變差。而有界變差和$$L^p$$空間的關聯在於$$L^1 \supseteq L^2 \supseteq \dots \supseteq L^\infty$$，因此如果函數$$g$$的一階有界變差不存在時，可以往二階或更高階變差討論(通常二階變差就會存在，如Brownian運動)。<mark style="color:red;">函式如果是有界變差的，那它在某種意義上可以被「很好地積分」</mark>。

有界變差函式提供了一種比連續函式更強的結構性質，但又比可微函式更廣泛的函式類別。有界變差函式可以被分解為兩個單調函式之差（Jordan 分解定理），這使得我們能夠對函式的行為進行更精細的分析。這種分解對於理解函式的區域性性質和整體性質非常有用。

在積分理論中，特別是 Riemann-Stieltjes 積分或 Lebesgue-Stieltjes 積分，有界變差函式扮演了重要角色。這些積分形式通常用於處理非光滑函式的情況，而有界變差函式正是這類函式的一個自然範疇。

如果$$f(x)$$ 是有界變差函式，那麼對於任何連續函式$$g(x)$$，Riemann-Stieltjes 積分$$\int_a^b​g(x)df(x)$$ 都是良好定義的。

在訊號處理中，訊號可能包含跳躍不連續點（如脈衝訊號），這些訊號通常是有限變差的。在控制理論中，系統的輸入或輸出可能具有有界變差的性質，這對於穩定性和可控性的分析至關重要。在逼近理論中，有界變差函式的性質可以用來分析多項式逼近或其他逼近方法的誤差。

### 要求函式的「良好性」

有界變差函式的總變差是有限的，這意味著函式的圖形在垂直方向上的總「擺動」是有限的。這種性質排除了病態的(如Weierstrass function)、劇烈震盪的函式，使得我們能夠處理更「良好」的函式類別。例如，函式 $$f(x)=sin(1/x)$$ 在包含 0 的區間上不是有界變差的，因為它在 0 附近無限震盪。

### 與單調函式的聯絡

一個重要的結果是，定義在閉區間上的實值函式是有界變差的，當且僅當它可以表示為兩個單調遞增函式的差。這個 **Jordan 分解定理** 提供了一個理解有界變差函式結構的重要工具，並允許我們將針對單調函式建立的理論推廣到有界變差函式。
