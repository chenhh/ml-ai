...
在處理積分$$\int f dg$$時，積分函數$$g$$的值域會切無限多段後，再進行加總。為了討論此切值域再加總的性質，所以討論有界變差。而有界變差和$$L^p$$空間的關聯在於$$L^1 \supseteq L^2 \supseteq \dots \supseteq L^\infty$$，因此如果函數$$g$$的一階有界變差不存在時，可以往二階或更高階變差討論(通常二階變差就會存在，如Brownian運動)。
