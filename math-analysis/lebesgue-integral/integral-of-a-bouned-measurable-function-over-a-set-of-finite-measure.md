...
<mark style="color:red;">包含了有界可測簡單函數與有界可測函數在有限測度集的(Lebesgue)積分。有界函數在有限測度集若可(Riemann)積分(不含瑕積分)，則等於Lebesgue積分</mark>。

有界可測(實值)函數，且定義域的測度有限，可用簡單函數由上/下逼近函數，再定義函數上/下積分相等時積分存在。
