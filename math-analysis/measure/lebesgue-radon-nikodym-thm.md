...
> 令$$\mu, \nu$$為可測空間$$(X, \Sigma)$$上的σ有限(符號)測度(可以分解為可數個有限測度的聯集)。
>
> 則存在唯一的σ有限(符號)測度$$\nu_{ac}, \nu_s$$滿足：
>
> 1. $$\nu=\nu_{ac} + \nu_{s}$$。(<mark style="color:red;">Lebesgue分解(decomposition</mark>)
> 2. $$\nu_{ac} \ll \mu$$ (絕對連續，即$$\forall E \in \Sigma, ~\mu(E)=0 \implies \nu_{ac}(E)=0$$)
> 3. $$\nu_s \perp \mu$$ (相互奇異(正交)，即$$\exists E, F \in \Sigma, E \cup F =X, E\cap F=\emptyset, \nu_s(E)=0, \mu(F)=0$$且$$E$$為$$\nu_s$$的零測集，$$F$$為$$\mu$$的零測集或者寫成$$\exists E \in \Sigma \ni$$$$\nu_s(E)=0$$且$$\mu(X \setminus E)=0$$)。
>
> σ有限 $$\nu$$測度指的是對於任意可測分割$$\bigcup_{n=1}^\infty E_n = X$$，$$\nu(E_n) < \infty, ~\forall n$$，但$$\nu(X)$$可能為$$\infty$$。因此可得有限測度為σ有限測度的子集合。<mark style="background-color:red;">換句話說，σ-有限測度可以被看作是可以用可數個有限測度的集合來近似描述的測度</mark>。
>
> Radon–Nikodym–Lebesgue 定理把測度沿著 σ-有限測度分解為絕對連續部分與奇異部分，並給出絕對連續部分的分類。
>
> Lebesgue 分解的核心依賴於 Radon-Nikodym 定理，而 Radon-Nikodym 定理本身要求測度$$\nu$$和$$\mu$$滿足 σ-有限性條件。
>
> 有界變差函數可以在精確到加減常數的意義下唯一分解為一個絕對連續函數、一個奇異函數和一個跳躍函數的和。

這個定理幫助我們理解一個測度相對於另一個測度的結構。一個測度可能是絕對連續的，或者可能是奇異的，或者可以分解成這兩種性質的部分組合。

聯繫積分和測度： Lebesgue定理是證明Radon-Nikodym定理 的關鍵步驟。如果$$\nu$$ 相對於$$\mu$$是絕對連續的，那麼存在一個可測函數$$f$$ (稱為Radon-Nikodym導數，記為 $$\d\nu/d\mu$$)，使得對於任何可測集合$$E$$，$$\nu(E)=\int_E f d\mu$$。這就建立了測度和積分之間的深刻聯繫。

<mark style="background-color:red;">總結來說，Lebesgue分解定理是一個強大的工具，它將一個測度分解成一個與另一個測度「平滑」相關的部分（絕對連續部分）和一個與之「完全不同」的部分（奇異部分），並且這種分解是唯一的</mark>。
...
* 如果$$\nu$$是由函數$$f(x)$$ 定義的測度（即 $$\nu(A) = \int_A f(x)  dx$$），那麼$$\nu$$ 是完全絕對連續的。
* 如果$$ν$$ 是集中在某些離散點上的測度（例如$$\nu({x_1}) = p_1, \nu({x_2}) = p_2, \dots$$），那麼$$\nu$$ 是完全奇異的。
* 更一般的情況下，$$ν$$可能同時包含絕對連續部分和奇異部分。例如$$\nu(A)=\int_A f(x)dx + \sum_{i=1}^n p_i \delta_{x_i}(A)$$。

應用：

* 在機率論中，它可以用來分解一個機率測度相對於另一個機率測度的連續和離散部分。
* 在訊號處理中，它幫助分析訊號的連續和奇異成分。
