...
切割法簡單的說，就是在直線(實數)上任意選一點$$r$$切割(可能為無理數或有理數)，依該點將實數分成左、右兩個集合。

* 將小於$$r$$(一定不會包含$$r$$)左側的全體有理數且沒有最大值的集合稱為左分割$$S$$。
* 而將大於$$r$$(可能會包含$$r$$)右則的全體有理數形成的集合稱為右分割$$T \equiv (\mathbb{Q} \setminus S$$)。
