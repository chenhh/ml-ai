...
註：級數收斂的必要條件是序列要收斂至0，而實數中的收斂數列的充要條件是Cauchy序列。
...
若級數$$\displaystyle \sum_{k=1}^\infty |x_k |$$收斂，則稱級數  $$\displaystyle \sum_{k=1}^\infty x_k$$為<mark style="color:red;">絕對收斂（absolute convergence）</mark>。

若級數$$\displaystyle \sum_{k=1}^\infty |x_k |$$發散，但級數$$\displaystyle \sum_{k=1}^\infty x_k$$收斂，則稱級數$$\displaystyle \sum_{k=1}^\infty x_k$$<mark style="color:red;">條件收斂（conditional convergence）</mark>。
