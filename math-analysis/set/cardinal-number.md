...
>
> 等勢集合有遞移性，即$$X \sim Y$$，且$$Y \sim Z$$，則$$X \sim Z$$。
...
$$P(A)$$ 為$$A$$的所有子集形成的集合。只要證明所有的函數$$f:A \rightarrow P(A)$$均不為映成函數即可。

令$$B= \{ a\in A | a\notin f(a)\}$$，則$$B \subset A$$為子集合，因此$$B \in P(A)$$且$$B \neq f(a)$$。

由定義得$$\forall b \in A \setminus B$$, $$b \notin B$$且 $$b \in f(b)$$，所以$$B \neq f(b)$$。 因此可得$$B \notin f(A)$$，因此$$f$$不是映成函數 (QED)。

</details>

### 使用函數判斷集合勢的大小

> 如果存在從$$X$$到$$Y$$的一個單射（一對一映射），則集合$$Y$$大於等於一個集合$$X$$， 記為$$|Y| \geq |X|$$，或稱集合$$Y$$有大於等於$$X$$的勢。
>
...
![一對一函數中，對應域可能存在元素沒有被對應](../../.gitbook/assets/1-1_function-min.png)
...
> 對於任意的集合$$X$$, $$|X| < |\mathcal{P}(X)|$$(無最大基數集合)

### 有限集合的基數

#### 有限基數

> 令$$n \in \mathbb{N}$$, 一個集合$$S$$的基數為$$n$$若且唯若該集合與 集合$$\{i \in \mathbb{N}~|~ i \leq i \leq n \}$$等價。 註：基數具有唯一性。
...
> 集合$$S$$為有限集若且唯且其基數$$|S| < \infty$$，否則$$S$$為無限集。 註：由定義可得自然數$$\mathbb{N}$$為無限集合。
...
> 令$$S_1, S_2, \cdots, S_n$$均為有限集合， 令$$\displaystyle |\bigcup_{i=1}^n S_i| > n$$， 則存在至少一集合的基數大於等於2，即$$\exists i \in \{1,\dots, n\} \ni |S_i| \geq 2$$。

<details>

<summary>proof: 反證法</summary>

令$$S_1, S_2, \cdots, S_n$$的基數全部小於2，因此$$S_i, ~\forall i$$為空集合或是單點集合。 因此$$|\bigcup_{i=1}^n S_i| \leq n$$ (QED)
...
* <mark style="color:red;">**一個無限集合可以和它的一個真子集有相同的基數**</mark>。
  * 如$$\{1,2,\cdots, n,\cdots\}$$ 與 $$\{2,4,\cdots, 2n, \cdots\}$$基數相同，因為在兩個集合之間，存在$$f(n)=2n$$為1-1且onto的函數。
...
若集合$$X,Y$$皆非空集合且其中之一為無限集，則 $$|X| + |Y| = |X||Y| = \max \{|X|, |Y| \}$$
...
## 連續統假設(Continuum hypothesis)

\[[Wikepedia](https://zh.m.wikipedia.org/zh-tw/%E8%BF%9E%E7%BB%AD%E7%BB%9F%E5%81%87%E8%AE%BE)] 由Georg Cantor於1878年提出：

> 不存在一個集合，使得它的基數(Cardinality)嚴格介於實數集$$|\mathbb{R}|=\aleph_1$$與自然數集$$|\mathbb{N}|=\aleph_0$$之間。$$\aleph_1=2^{\aleph_0}$$
>
> 即不存在集合$$S$$滿足$$\aleph_0 < |S| < \aleph_1$$。
