...
<mark style="color:red;">定義sigma域（代數）的主要用途，是在定義實值可測函數（實值隨機變數）的前像，必須為sigma域內的元素才符合可測函數的定義。並非</mark>定義域中的任意集合(power set)均可定義隨機變數。
...
filtration可以看作一個資訊庫，而且這個資訊庫並不是固定的，它會隨著時間t而增加里面的資訊量。

對於隨機過程的一條路徑，從時間0到某個時間$$t$$ ，有一些事件是可以被確定的，有一些則不能，filtration把可以被確定的那些事件選出來。
