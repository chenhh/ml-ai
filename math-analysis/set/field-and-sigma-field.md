...
<mark style="color:red;">σ域</mark>另一個用途是定義隨機過程中的濾流(filitration)$$\{\mathcal{F}_t\}$$，或稱為資訊集合；簡單的說就是在時間$$t$$的隨機過程中的變數$$X_t$$只能使用已發生的歷史資訊$$\mathcal{F}_t$$來決定未來的變化，稱為$$X_t$$適應於$$\mathcal{F}_t$$；在隨機規劃中，使用情境樹(scenario tree)表示資訊集合。

因為給定任意的集合時，依公理可以建構出不可測的集合族（set family，元素為集合的集合），為了從冪集合中排除這類性質不佳的集合族，因此才定義σ域，而日常所使用的集合，幾乎都滿足σ域的條件，所以可放心使用。
...
> $$F_1, F_2$$均為σ環，則$$F_1 \cap F_2$$為σ環
