...
註：在賽局理論中，行動為$$n$$個行動的線性組合指的是<mark style="color:red;">混合策略（mixed strategy）</mark>，即第$$i$$個行動被選中的機率為$$x_i$$。行動被選中的機率符從[Dirichlet分佈](https://en.wikipedia.org/wiki/Dirichlet_distribution)。
