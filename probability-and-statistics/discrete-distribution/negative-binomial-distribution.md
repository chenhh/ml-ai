...
>
> 機率質量函數$$f(k | r,p)≡\mathrm{P}⁡(X=k)=\binom{k+r-1}{k}p^r(1-p)^k, ~ k=0,1,2,\ldots$$
>
...
其中$$\begin{aligned} \binom{k+r-1}{r-1} &= \frac{(k+r-1)!}{k! (r-1)!} \\ & =\frac{(k+r-1)(k+r-2)\dots (r)}{k!} \\ & = (-1)^k \frac{(-r)(-r-1)(-r-2) \dots (-r-k-1)}{k!} \\ & = (-1)^k \binom{-r}{k}  \end{aligned}$$
...
>
> 其中$$n$$為第$$r$$次成功的總試驗次數，因此在$$n-1$$次試驗中，為$$r-1$$次成功和$$n-r$$次失敗的排列組合，共有$$\binom{n-1}{r-1}$$種組合。
