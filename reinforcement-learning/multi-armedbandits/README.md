...
在吃角子老虎機問題中，$$k$$個行動都有一個預期的或平均的獎勵，可稱為該行動的價值(value)。我們用$$A_t$$(隨機變數)表示在時間步驟t上選擇的行動，用$$R_t$$(隨機變數)表示相應行動的報酬。 那麼，任意行動$$a$$(實現值)的期望價值可表示為$$q_{*}(a)$$，是指在$$a$$被選中的情況下的期望報酬：

$$q_{*}(a) \equiv \mathrm{E}(R_t ~|~ A_t = a)$$

注意此處的期望報酬是在第$$t$$期決策選擇行動$$a$$的期望值(真實分佈的期望值，但通常未知)，而不是當期的實現值。
